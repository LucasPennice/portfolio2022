...
function Text({ data }: { data: WorkExperience }) {
  ...
  return (
    <aside className="flex flex-col sticky top-14 cursor-default">
      {companyNameWords.map((word, idx) => {
        return (
          <div
            key={word}
            className="relative overflow-clip hoverUnderlineDiv"
...
function Underline({ idx }: { idx: number }) {
  return (
    <m.div
      {...appearTextUnderlineAnimation(idx)}
      style={{ height: 7 }}
      className="w-full absolute bottom-0 left-0 bg-black"
    ></m.div>
  );
}
