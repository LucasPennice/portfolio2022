...
function MobileMenu({ layoutState }: { layoutState: LayoutState }) {
  ...
  return (
    <m.div
      ...
      animate={currentlyViewing === PortfolioMode.MobileMenu ? "active" : "inactive"}
      variants={calculateVariants(currentlyViewing, PortfolioMode.MobileMenu)}
      {...layoutAnimationSettings}
    >
      mobileMenu
      <button onClick={() => setCurrentlyViewing(PortfolioMode.Main)}>close</button>
    </m.div>
  );
}
